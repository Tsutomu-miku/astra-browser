...
// -- Tab identity metadata keys ---------------------------------------------
//
// Stable tab unique identifier that persists across navigations, discards,
// and session restores.  Generated by AstraTabFeatures via
// base::UnguessableToken.
//
// Chromium owner: AstraTabFeatures (WebContentsUserData).
// Serialized as a string representation of the UnguessableToken.
inline constexpr char kMetaKeyTabUniqueId[] = "astra.tab_unique_id";

// -- Source workspace metadata keys -----------------------------------------
//
// The workspace this tab was originally in before being moved to its current
// workspace.  Used for "move back" operations and tab provenance tracking.
inline constexpr char kMetaKeySourceWorkspaceId[] = "astra.source_workspace_id";

// -- Tab color metadata keys ------------------------------------------------
//
// Per-tab color for visual grouping and identification.
// Stored as a uint32_t (ARGB SkColor value).
inline constexpr char kMetaKeyTabColor[] = "astra.tab_color";

// -- Read later metadata keys -----------------------------------------------
//
// Astra-native "read later" flag, distinct from Chromium's ReadingListModel.
// Marks tabs that the user has saved to read later but haven't been added
// to the formal reading list.
inline constexpr char kMetaKeyReadLater[] = "astra.read_later";

// -- Snooze metadata keys ---------------------------------------------------
//
// Snoozed tabs are temporarily hidden and resurface at a scheduled time.
// is_snoozed indicates the tab is currently snoozed.
// snooze_time is when the tab should wake up (microseconds since Windows epoch,
// stored as double for base::Time serialization compatibility).
inline constexpr char kMetaKeyIsSnoozed[] = "astra.is_snoozed";
inline constexpr char kMetaKeySnoozeTime[] = "astra.snooze_time";

// -- Tab hibernation metadata keys ------------------------------------------
//
// Per-tab hibernation flag.  Hibernated tabs are unloaded from memory but
// kept in the tab strip (similar to Chromium's tab discard but user-initiated
// and Astra-managed).
//
// Note: This is per-tab hibernation state.  Window-level hibernation
// (kMetaKeyWindowIsHibernated) controls whether an entire window is hibernated.
inline constexpr char kMetaKeyIsHibernatedTab[] = "astra.is_hibernated_tab";

// -- Tab created time metadata keys -----------------------------------------
//
// When the tab was originally created (wall-clock time).
// Stored as double (microseconds since Windows epoch) for consistency with
// last_active_time serialization.
inline constexpr char kMetaKeyCreatedTime[] = "astra.created_time";
...
struct AstraTabSessionMetadata {
  ...
  // Stable unique identifier for this tab (base::UnguessableToken as string).
  // Persists across navigations, discards, and session restores.
  std::string tab_unique_id;

  // Workspace this tab was originally created in / moved from.
  std::string source_workspace_id;

  // Tab color (SkColor stored as uint32_t, 0 means no color).
  uint32_t tab_color = 0;

  // -- Astra-native tab states -----------------------------------------------

  // Read later flag (Astra-native, distinct from Chromium's ReadingListModel).
  ...
  // Per-tab hibernation state (user-initiated, Astra-managed).
  bool is_hibernated = false;

  // Created time (wall-clock time of tab creation).
