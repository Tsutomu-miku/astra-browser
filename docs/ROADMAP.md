...
Latest P1-1 note: sidebar menu surface chrome now lives in the shared sidebar menu stylesheet, menu focus/keyboard/context-menu event behavior now lives in a shared sidebar menu surface component, and menu dismissal listeners now live in a shared context-menu hook. Tab, quick-entry, group, closed-tab, Space, and anchored menus use the same floating surface, viewport clipping, overflow behavior, Escape/click/blur/scroll dismissal, keyboard navigation, and click isolation while keeping only width differences in section CSS.
