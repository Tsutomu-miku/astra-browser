...
Latest P1-1 note: sidebar menu surface chrome now lives in the shared sidebar menu stylesheet and menu focus/keyboard/context-menu event behavior now lives in a shared sidebar menu surface component, so tab, quick-entry, group, closed-tab, and Space menus use the same floating surface, viewport clipping, overflow behavior, keyboard navigation, and click isolation while keeping only width differences in section CSS.
