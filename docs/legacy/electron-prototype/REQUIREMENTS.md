...
- Sidebar Space rail buttons must expose active and drop-target state in accessible labels, including tab and New Space drop targets.
...
- Sidebar folder drops for Tabs, Pinned, and Favorites should use one tab-folder handler; quick-entry Favorite drag logic is only for URL-only legacy entries and must not synthesize tab payloads.
...
- Tab-backed Space favorite reordering must use the same tab-row drag path as other sidebar folders; URL-only legacy Favorites may remain on the quick-entry fallback path.
- Users must be able to drag tab-backed Space favorites onto Space buttons or the New Space button as normal tabs, preserving their Favorites folder membership in the destination Space.
- Users must be able to drag URL-only legacy Space favorites onto Space buttons or the New Space button to move them between Spaces.
...
- Sidebar search tab-backed Favorite targets must preview and split with the backing tab's current title and URL.
...
- Direct tab selection must support Zen-style `Alt+1` through `Alt+8`, ordered by Essentials, pinned tabs, tab-backed Favorites, then regular tabs.
...
- The new tab page must follow the global chrome accent mode, using the neutral chrome accent by default and the Space accent only when that setting is enabled.
- New tab quick-entry tiles must use the same site favicon cache and fallback icon model as sidebar rows.
