...
`src/renderer/surfaces/webview` keeps all current Space webviews mounted and hides inactive tabs instead of unmounting them. This preserves Chromium page state during ordinary tab switching while still rendering split view as the two visible webviews first.
