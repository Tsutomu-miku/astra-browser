...
Sidebar drag-and-drop identity resolution lives in `src/renderer/surfaces/sidebar/model/sidebarDragSources.ts`. Components and hooks should not hard-code `DataTransfer` keys for tabs, groups, Essentials, Favorites, closed tabs, or workspaces; they should ask this model for the dragged source and then apply their own target-specific acceptance rules. Space and New Space drop dispatch is further normalized through `sidebarWorkspaceDropIntent.ts`, so legacy Favorites, tabs, tab groups, closed tabs, and Space reordering are not handled by competing component branches. This keeps Spaces, New Space, Split, tab folders, groups, Essentials, and legacy quick-entry drops aligned when React drag state has not synced yet.
